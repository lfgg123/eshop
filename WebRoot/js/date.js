...
(function ($) {      
	...
	function resetIndex(){
        ...
        function submitBirthDay(datestr){
        	//alert(datestr);
            //var birth= new Date(Date.parse(datestr.replace(/-/g,  "/")));   
            var userId=$("#userId").val();
            //后台请求
            $.ajax({
            	//url:path+'app/member/getMemberInfo',
                url: path + 'app/member/modifyBirth',
                type: "GET",
                data:{userId:userId, birth:datestr,TOKEN:$("#token").val()},
                dataType: "JSON",
                cache: false,
                async: false,
        ...
        function bindButton(){
           ...
           var btnArray = ['确认', '取消'];
           var cflag=true;
            mui.confirm('确定修改？',btnArray, function(e) {
                    if (e.index==1) {
                       $("#datePage").hide(); 
                       $("#dateshadow").hide();
                      cflag=false;
